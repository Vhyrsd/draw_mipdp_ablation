...
try:
    ...
    # rolling mean
    window_size = 3
    plt.plot(df['Step'], df['miloss0.01_mitemp0.5_15/50_2022.12.29-22.31.27_train_diffusion_unet_hybrid_lift_image - test/mean_score'].rolling(
        window=window_size, min_periods=1).mean(), label='$\\tau=0.5$', marker='', linestyle='--', color='b')
    plt.plot(df['Step'], df['miloss0.01_mitemp0.2_15/50_2022.12.29-22.31.27_train_diffusion_unet_hybrid_lift_image - test/mean_score'].rolling(
        window=window_size, min_periods=1).mean(), label='$\\tau=0.2$', marker='', linestyle='-', color='r')
    plt.plot(df['Step'], df['miloss0.01_mitemp0.1_15/50_2022.12.29-22.31.27_train_diffusion_unet_hybrid_lift_image - test/mean_score'].rolling(
        window=window_size, min_periods=1).mean(), label='$\\tau=0.1$ (Ours)', marker='', linestyle=':', color='c')
    plt.plot(df['Step'], df['miloss0.01_mitemp0.05_15/50_2022.12.29-22.31.27_train_diffusion_unet_hybrid_lift_image - test/mean_score'].rolling(
        window=window_size, min_periods=1).mean(), label='$\\tau=0.05$', marker='', linestyle='-.', color='g')

    # 6. 添加图表细节
    # plt.title('MI temperature', fontsize=16)  # 标题
