...
def load_features(feature_path):
    ...
    for ep_idx, episode in enumerate(data['episodes']):
        ...
        if isinstance(features, np.ndarray):
            if features.size == 0:
                print(f"Warning: Episode {ep_idx} (env {env_id}) has empty features, skipping")
                continue
            ...
            # 如果特征是字典，只取第一个特征
            if len(features) == 0:
                print(f"Warning: Episode {ep_idx} (env {env_id}) has empty feature dict, skipping")
                continue
            ...
            if feat_array.size == 0:
                print(f"Warning: Episode {ep_idx} (env {env_id}) has empty features[{key}], skipping")
                continue
            ...
            print(f"Warning: Episode {ep_idx} (env {env_id}) has unknown feature type: {type(features)}")
            continue
    
    # 检查是否有有效特征
    if len(all_features) == 0:
        raise ValueError("No valid features found! Check if feature extraction was successful.")
