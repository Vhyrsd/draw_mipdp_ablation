...
def compute_metrics(features, env_ids, prefixes):
    ...
    try:
        metrics['silhouette_env'] = silhouette_score(features, env_ids, sample_size=min(10000, len(features)))
    except:
    ...
    # 衡量簇间方差与簇内方差的比值
    try:
        metrics['calinski_harabasz'] = calinski_harabasz_score(features, env_ids)
    except:
    ...
    if len(env_centers) > 1:
        inter_env_distances = cdist(env_centers, env_centers, metric='euclidean')
        # 只取上三角（不包括对角线）
...
def main(baseline_dir, ours_dir, output):
    ...
    print("\nComputing metrics...")
    baseline_metrics = compute_metrics(baseline_features, baseline_env_ids, baseline_prefixes)
    ...
    for key in baseline_metrics.keys():
        ...
        if not np.isnan(baseline_val) and not np.isnan(ours_val):
            ...
            if key in ['davies_bouldin', 'trajectory_smoothness', 'test_to_train_dist_mean']:
                ...
                improvement = (ours_val - baseline_val) / abs(baseline_val) * 100
                better = "✓" if improvement > 0 else "✗"
            
            print(f"{key:<35} {baseline_val:<15.4f} {ours_val:<15.4f} {better} {improvement:>+6.2f}%")
    ...
    bars1 = ax1.bar(x - width/2, baseline_vals_norm, width, label='Baseline', alpha=0.8)
    bars2 = ax1.bar(x + width/2, ours_vals_norm, width, label='Ours (MI-enhanced)', alpha=0.8)
    
    ax1.set_ylabel('Normalized Score')
    ax1.set_title('Key Metrics Comparison (Normalized)', fontsize=14, fontweight='bold')
    ax1.set_xticks(x)
    ...
    smoothness_labels = ['Trajectory Smoothness\n(lower better)', 'Davies-Bouldin Index\n(lower better)']
    ...
    bars1 = ax2.bar(x2 - width/2, baseline_smooth_norm, width, label='Baseline', alpha=0.8, color='coral')
    bars2 = ax2.bar(x2 + width/2, ours_smooth_norm, width, label='Ours (MI-enhanced)', alpha=0.8, color='skyblue')
    
    ax2.set_ylabel('Normalized Score')
    ax2.set_title('Smoothness Metrics (Lower is Better)', fontsize=14, fontweight='bold')
    ax2.set_xticks(x2)
    ...
    baseline_2d = tsne_baseline.fit_transform(baseline_features[:5000])  # 子采样加速
    scatter1 = ax3.scatter(baseline_2d[:, 0], baseline_2d[:, 1], 
                          c=baseline_env_ids[:5000], cmap='tab20', alpha=0.6, s=10)
    ax3.set_title('Baseline: t-SNE by Environment ID', fontsize=12, fontweight='bold')
    ...
    ax4.set_title('Ours (MI-enhanced): t-SNE by Environment ID', fontsize=12, fontweight='bold')
    ...
    ax5.set_title(f'Baseline: Trajectory Example (Env {sample_env})', fontsize=12, fontweight='bold')
    ax5.legend()
    ...
    if mask_ours.sum() > 0:
        ...
        ax6.scatter(traj_ours[-1, 0], traj_ours[-1, 1], 
                   s=200, marker='s', color='red', edgecolors='black', linewidths=2, label='End', zorder=5)
    ax6.set_title(f'Ours (MI-enhanced): Trajectory Example (Env {sample_env})', fontsize=12, fontweight='bold')
    ax6.legend()
    ...
    with open(output_txt, 'w') as f:
        ...
        f.write(f"{'Metric':<35} {'Baseline':<15} {'Ours':<15} {'Improvement':<15}\n")
        f.write("-"*80 + "\n")
        ...
        for key in baseline_metrics.keys():
            ...
            if not np.isnan(baseline_val) and not np.isnan(ours_val):
                ...
                f.write(f"{key:<35} {baseline_val:<15.4f} {ours_val:<15.4f} {better} {improvement:>+6.2f}%\n")
